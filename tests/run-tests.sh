...
"$NODE" --check "$ROOT/website/qq/site.js" >/dev/null

if ! /usr/bin/grep -F -q '支持 macOS 与 Windows' "$ROOT/website/index.html" \
  || ! /usr/bin/grep -F -q 'src="../qq-system-cough.mp3"' "$ROOT/website/qq/index.html" \
...
for preset in "$ROOT"/presets/preset-*/; do
  ...
  "$NODE" -e '
    ...
    if (!v.pass || !String(v.themeId).startsWith("preset-") || v.imageBytes < 1 ||
      v.petBytes < 1 || v.frameBytes < 1 || v.qqAvatarBytes < 1) process.exit(1);
  ' "$PRESET_CHECK"
done
...
"$NODE" "$ROOT/scripts/injector.mjs" --watch --port 19341 --theme-dir "$ROOT/presets/preset-classic-codex" \
...
"$NODE" -e '
  ...
  if (!value.pass || value.themeName !== "测试主题" || value.imageBytes < 1 ||
    value.petBytes < 1 || value.frameBytes < 1 || value.qqAvatarBytes < 1) process.exit(1);
...
assert_write_theme_rejected layout-mode --layout-mode floating
assert_write_theme_rejected right-panel --right-panel toggle
assert_write_theme_rejected layout-min-width --layout-min-width 1079
assert_write_theme_rejected right-panel-width --right-panel-width 361
...
"$NODE" -e '
  ...
  const cases = {
    ...
    "layout-mode": (theme) => { theme.layout.mode = "floating"; },
    "right-panel": (theme) => { theme.layout.rightPanel = "toggle"; },
    "layout-min-width": (theme) => { theme.layout.minWidth = 1079; },
    "right-panel-width": (theme) => { theme.layout.rightWidth = 361; },
...
for invalid_case in appearance safe-area task-mode focus-x focus-y layout-mode right-panel layout-min-width right-panel-width name-control; do
  if INVALID_OUTPUT="$(
  ...
  case "$invalid_case" in
    ...
    layout-mode) EXPECTED_INVALID_FIELD='layout.mode' ;;
    right-panel) EXPECTED_INVALID_FIELD='layout.rightPanel' ;;
    layout-min-width) EXPECTED_INVALID_FIELD='layout.minWidth' ;;
    right-panel-width) EXPECTED_INVALID_FIELD='layout.rightWidth' ;;
...
/usr/bin/env -u HOME /bin/bash -c '. "$1/scripts/common-macos.sh"; [ -n "$HOME" ] && [ "$SKIN_VERSION" = "2.6.3" ]' _ "$ROOT"
