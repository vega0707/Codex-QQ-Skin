...
- 根据活跃日和每日总 token 档位计算成长值，以 QQ 经典的星星、月亮、太阳、皇冠显示等级。
- 点击右上角“资料”可恢复 Codex 原生输出/来源面板，再点“成长统计”即可返回。
- 统计直接读取本机 `~/.codex/sessions` 与 `archived_sessions`，使用增量缓存；不需要额外账号，不读取 API Key，不上传 prompt 或 token 数据。
...
- **伙伴房间：** 右下角 Codex 伙伴升级为会随任务状态、时间与互动变化的桌面房间；电脑、书架、台灯和盆栽都可打开 GitHub 热门项目盲盒。
- **项目盲盒：** 机器人先从书架抽书，再随机展示热门 GitHub 项目的名称与中文简介；支持收藏、打开仓库、换一本和减少同类推荐。
- **本地成长：** 每连续发现 5 个项目解锁一件房间摆件；收藏、偏好、发现次数和摆件进度只保存在本机。
- **可靠交互：** 盲盒按钮使用独立 `pointerup + click` 双通道，适配 Electron 对标准点击事件的偶发吞噬。
- **安全数据源：** 项目列表只读取 GitHub 公共仓库搜索结果，每 6 小时缓存一次；项目简介自动翻译为中文，翻译失败时使用中文兜底，不上传 Codex 对话或本机统计。
...
- 右上默认显示本机 Codex 成长中心；点击“资料”可无损切回真实“输出 / 来源 / 进度 / 子代理”。右下 Codex 伙伴继续提供打开宠物、终端、提示音三个快捷按钮，并显示任务状态与本周剩余用量。
- 伙伴房间里的电脑、书架、台灯和盆栽会打开 GitHub 热门项目盲盒；机器人先执行抽书动作，再展示项目名称和中文简介。
- 盲盒支持收藏、打开 GitHub、换一本与“以后少推荐这种”；连续发现 5 个项目会解锁房间摆件，全部偏好与进度仅写入浏览器本地存储。
...
测试覆盖注入 payload、图片元数据、主题切换、项目盲盒数据缓存与交互、UTF-8 配置往返、回环 CDP 限制、清理恢复和官方签名检查。
